...
static const struct ipahal_reg_obj ipahal_reg_objs[][IPA_REG_MAX] = {
	/* IPAv3 */
	[IPA_HW_v3_0] = {
		reg_obj_cfunc(ROUTE, route,		0x00000048,	0),
		...
		reg_obj_nofunc(BCR,			0x000001d0,	0),
		reg_obj_nofunc(ENABLED_PIPES,		0x00000038,	0),
		reg_obj_nofunc(COMP_SW_RESET,		0x00000040,	0),
		reg_obj_nofunc(VERSION,			0x00000034,	0),
		reg_obj_nofunc(TAG_TIMER,		0x00000060,	0),
		reg_obj_nofunc(COMP_HW_VERSION,		0x00000030,	0),
		reg_obj_nofunc(SPARE_REG_1,		0x00005090,	0),
		reg_obj_nofunc(SPARE_REG_2,		0x00005094,	0),
		reg_obj_nofunc(COMP_CFG,		0x0000003c,	0),
		reg_obj_nofunc(STATE_AGGR_ACTIVE,	0x0000010c,	0),
		...
		reg_obj_both(ENDP_INIT_AGGR_n, endp_init_aggr_n,
							0x00000824,	0x70),
		reg_obj_nofunc(AGGR_FORCE_CLOSE,	0x000001ec,	0),
		...
		reg_obj_cfunc(ENDP_INIT_DEAGGR_n, endp_init_deaggr_n,
							0x00000834,	0x70),
		reg_obj_nofunc(ENDP_INIT_SEQ_n,		0x0000083c,	0x70),
		reg_obj_nofunc(DEBUG_CNT_REG_n,		0x00000600,	0x4),
		...
		reg_obj_pfunc(SHARED_MEM_SIZE, shared_mem_size,
							0x00000054,	0),
		reg_obj_nofunc(SRAM_DIRECT_ACCESS_n,	0x00007000,	0x4),
		reg_obj_cfunc(DEBUG_CNT_CTRL_n, debug_cnt_ctrl_n,
							0x00000640,	0x4),
		reg_obj_nofunc(UC_MAILBOX_m_n,		0x00032000,	0x4),
		reg_obj_nofunc(FILT_ROUT_HASH_FLUSH,	0x00000090,	0),
		reg_obj_both(SINGLE_NDP_MODE, single_ndp_mode,
							0x00000068,	0),
		reg_obj_both(QCNCM, qcncm,		0x00000064,	0),
		reg_obj_nofunc(SYS_PKT_PROC_CNTXT_BASE,	0x000001e0,	0),
		...
		reg_obj_cfunc(QSB_MAX_READS, qsb_max_reads,
							0x00000078,	0),
		reg_obj_nofunc(DPS_SEQUENCER_FIRST,	0x0001e000,	0),
		reg_obj_nofunc(HPS_SEQUENCER_FIRST,	0x0001e080,	0),
	},
	...
	/* IPAv3.5 */
	[IPA_HW_v3_5] = {
		reg_obj_both(TX_CFG, tx_cfg,		0x000001fc,	0),
		...
		reg_obj_nofunc(RX_HPS_CLIENTS_MAX_DEPTH_1,
							OFFSET_INVAL,	0),
		reg_obj_nofunc(SPARE_REG_1,		0x00002780,	0),
		reg_obj_nofunc(SPARE_REG_2,		0x00002784,	0),
	...
	[IPA_HW_v4_0] = {
		reg_obj_cfunc(ENDP_INIT_CTRL_n, endp_init_ctrl_n_v4_0,
							0x00000800,	0x70),
		reg_obj_both(TX_CFG, tx_cfg_v4_0,	0x000001fc,	0),
		reg_obj_nofunc(DEBUG_CNT_REG_n,		OFFSET_INVAL,	0),
		reg_obj_cfunc(DEBUG_CNT_CTRL_n, debug_cnt_ctrl_n,
							OFFSET_INVAL,	0),
		reg_obj_both(QCNCM, qcncm,		OFFSET_INVAL,	0),
		reg_obj_both(SINGLE_NDP_MODE, single_ndp_mode,
							OFFSET_INVAL,	0),
		reg_obj_cfunc(QSB_MAX_READS, qsb_max_reads_v4_0,
							0x00000078,	0),
		reg_obj_nofunc(FILT_ROUT_HASH_FLUSH,	0x0000014c,	0),
		reg_obj_nofunc(STATE_AGGR_ACTIVE,	0x000000b4,	0),
		reg_obj_cfunc(ENDP_INIT_ROUTE_n, endp_init_route_n,
							OFFSET_INVAL,	0),
		reg_obj_cfunc(ENDP_STATUS_n, endp_status_n_v4_0,
							0x00000840,	0x70),
		reg_obj_nofunc(CLKON_CFG,		0x00000044,	0),
		reg_obj_cfunc(ENDP_INIT_CONN_TRACK_n, endp_init_conn_track_n,
							0x00000850,	0x70),
		reg_obj_nofunc(STAT_QUOTA_BASE_n,	0x00000700,	0x4),
		reg_obj_nofunc(STAT_QUOTA_MASK_n,	0x00000708,	0x4),
		reg_obj_nofunc(STAT_TETHERING_BASE_n,	0x00000710,	0x4),
		reg_obj_nofunc(STAT_TETHERING_MASK_n,	0x00000718,	0x4),
		reg_obj_nofunc(STAT_FILTER_IPV4_BASE,	0x00000720,	0x0),
		reg_obj_nofunc(STAT_FILTER_IPV6_BASE,	0x00000724,	0x0),
		reg_obj_nofunc(STAT_ROUTER_IPV4_BASE,	0x00000728,	0x0),
		reg_obj_nofunc(STAT_ROUTER_IPV6_BASE,	0x0000072c,	0x0),
		...
		reg_obj_nofunc(STAT_ROUTER_IPV6_END_ID,
							0x0000074c,	0x0),
		reg_obj_nofunc(STAT_DROP_CNT_BASE_n,	0x00000750,	0x4),

...
static int ath10k_qmi_msa_mem_info_send_sync_msg(struct ath10k_qmi *qmi)
{
	...
	for (i = 0; i < resp.mem_region_info_len; i++) {
		...
		ath10k_dbg(ar, ATH10K_DBG_QMI, "mem region: %d Addr: 0x%pa Size: 0x%x Flag: 0x%08x\n",
...
static int ath10k_qmi_setup_msa_resources(struct ath10k_qmi *qmi, u32 msa_size)
{
	...
	np = of_parse_phandle(dev->of_node, "qcom,msa-fixed-region", 0);
	if (np) {
		...
		qmi->msa_va = devm_memremap(dev, qmi->msa_pa,
